...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        if len(game.get_legal_moves(self)) > 0:
            best_move = game.get_legal_moves(self)[random.randint(0, len(game.get_legal_moves(self))-1)]
        else:
            best_move = (-1, -1)
        try:
            depth = 1
    ...
    def min_value(self,game,depth,alpha, beta):
        ...
        v = float("inf")
        for m in game.get_legal_moves():
            if self.max_value(game.forecast_move(m),depth-1,alpha,beta)[0] < v:
                v,_ = self.max_value(game.forecast_move(m),depth-1,alpha, beta)
                best_move = m
            if v <= alpha:
                return (v, best_move)
            beta = min(beta,v)
    ...
    def max_value(self,game,depth,alpha, beta):
        ...
        for m in game.get_legal_moves():
            ...
            if self.min_value(game.forecast_move(m),depth-1,alpha,beta)[0] > v:
                v,_ = self.min_value(game.forecast_move(m),depth-1,alpha, beta)
                best_move = m
            if v >= beta:
                return (v, best_move)
            alpha = max(alpha,v)
